...
function splitchild!(node::Node{Key}, childindex::Int) where Key
    ...
    if !leftnode.isleaf
        for j = 1:MIN_CHILDREN
            ...
            rightsum += rightnode.children[j].subtreesize
        end
    end
    ...
    if node.subtreesize == 0
        node.subtreesize = leftnode.subtreesize
    end
    rightnode.subtreesize = rightsum
    leftnode.subtreesize -= node.elements[childindex].counter
    leftnode.subtreesize -= rightsum
end
